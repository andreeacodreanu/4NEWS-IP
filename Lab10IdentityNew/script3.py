...
def main():
    ...
    for x in range(1, len(sys.argv)):
        article = article + " " + sys.argv[x]

    text1 = TextSummarizer()
    text1.text = article
    words = text1.tokenize_sentence()
    freqTable = text1.cal_freq(words)
    sentenceValue = text1.compute_sentence(freqTable)
    avg = text1.sumAvg(sentenceValue)
    summary = text1.print_summary(sentenceValue,avg)
    sum = str(summary)
    with open('C:\\Users\\Andreea\\Desktop\\Anul3\\ProiectIP2019\\Lab10IdentityNew\\file.txt', 'w') as f:
            f.write(sum.replace('’', '`').replace('—', '-').replace('“', '"').replace('”', '"').replace('é', 'e').replace('à', 'a'))
    f.close()
